<script setup lang="ts">
  ...
  const onUserSaved = async (user: CreateUserWithRoleIdData) => {
    if (userToEdit.value) {
      await usersApi.update(user as CreateUserWithRoleIdData)
      if (!error.value) {
        notify({
          message: `${user.name} ${user.lastName} ha sido actualizado`,
          color: 'success',
        })
      }
    } else {
      ...
      if (!error.value) {
        notify({
          message: `${user.name} ${user.lastName} ha sido creado`,
          color: 'success',
        })
      }
    }
...
<template>
  ...
  >
    <h1 class="va-h5">{{ userToEdit ? 'Editar usuario' : 'Añadir usuario' }}</h1>
    <EditUserForm
      ref="editFormRef"
      :user="userToEdit"
