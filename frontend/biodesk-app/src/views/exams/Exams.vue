<script setup lang="ts">
  ...
  async function mergePatientInfoIntoExams() {
    const uniqueHistoryIds = [
      ...new Set(exams.value.map((exam) => exam.medicHistoryId)),
    ];
    const patientInfoMap: Record<
      number,
      { ci: string; name: string; lastName: string }
    > = {};
    ...
    <VaCard>
      <VaCardContent>
        ...
        <div class="flex flex-col md:flex-row gap-2 mb-2 justify-between">
          ...
          >
            <VaInput
              ...
              placeholder="Search by medicHistoryId"
            >
        ...
        <VaModal v-model="showModal" hide-default-actions>
          <h2
            class="va-h3 text-primary"
            style="margin-top: 0; margin-bottom: 1.5rem; text-align: left"
          >
            Detalles del examen
          </h2>
          <div v-if="selectedExam">
            <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span>{{ selectedExam.lastName }}</span>
              </div>
              <div>
                <strong>Fecha de Solicitud:</strong>
                <span>{{ formatDate(selectedExam.requestedAt) }}</span>
              </div>
              <div>
          ...
          <div v-else>No exam selected or invalid data.</div>
        </VaModal>
